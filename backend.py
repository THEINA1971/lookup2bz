...
Talisman(
    ...
    == "true",  # Activé en production via variable d'environnement
...
def check_blocked_ip():
    ...
    if (
        ...
        log_attack(
            ip,
            "oversized_request",
            f"Requête trop volumineuse: {request.content_length} bytes",
        )
        return jsonify(
            {
...
def validate_request():
    ...
    if (
        ...
        and request.headers.get("X-Forwarded-Host") != request.host
    ):
...
def load_json(filepath):
    ...
    try:
        if not os.path.exists(filepath):
            ...
            default = (
                {}
                if "keys" in filepath or "users" in filepath or "sessions" in filepath
                else []
            )
            print(
                f"[INFO] Fichier {filepath} n'existe pas, utilisation de la valeur par défaut"
            )
            return default
...
def save_json(filepath, data):
    ...
    try:
        ...
        print(
            f"[WARNING] Erreur lors de la création du répertoire pour {filepath}: {e}"
        )
...
def send_verification_email(email, code):
    ...
    if not SMTP_ENABLED or not SMTP_USER or not SMTP_PASSWORD:
        ...
        print(
            f"⚠️  [DEV] L'envoi d'email est désactivé. Activez SMTP_ENABLED=true dans .env"
        )
        return False
...
def send_verification():
    ...
    response_data = {
        ...
        if email_sent
        else "Code de vérification généré (mode développement)",
    }
...
def register():
    ...
    if key_data.get("used_at") and not key_data.get("is_admin"):
        return jsonify(
            {"error": "Cette clé a déjà été utilisée pour une inscription"}
        ), 400

    if key_data.get("used_by") and not key_data.get("is_admin"):
        return jsonify(
            {"error": "Cette clé a déjà été utilisée par un autre utilisateur"}
        ), 400
    ...
    user_id = secrets.token_hex(16)
    username = (
        email.split("@")[0] + "_" + secrets.token_hex(4)
    ...
    if not key_data.get("is_admin"):
        key_data["used_by"] = (
            user_id  # Utilisateur qui a utilisé la clé pour s'inscrire
        )
...
def login():
    ...
    if not user.get("active", True):
        log_attack(
            ...
            f"Tentative de connexion à un compte désactivé: {email}",
        )
...
def get_keys():
    ...
    if is_admin_user:
        ...
        user_keys = {
            ...
            if v.get("created_by") == request.current_user_id
            or v.get("user_id") == request.current_user_id
        }
...
def create_key():
    ...
    is_admin_key = (
        data.get("is_admin", False) if is_admin_user else False
    )  # Seuls les admins peuvent créer des clés admin
    ...
    expires_at = (
        ...
        else (now + duration_map.get(duration, timedelta(days=1))).isoformat()
    )
    ...
    if is_admin_key:
        ...
        key_data[
            "usage_history"
        ] = []  # Historique des utilisations (clés admin réutilisables)
...
def delete_key(key_code):
    ...
    return jsonify({"success": True, "message": "Clé supprimée avec succès"}), 200
...
def check_key_status(key_code):
    ...
    if key_code not in keys:
        return jsonify({"exists": False, "error": "Clé introuvable"}), 404
    ...
    result = {
        ...
        "used_by_email": key_data.get("used_by_email")
        if status == "used"
        else None,  # Afficher l'email seulement si utilisée
...
def verify_key():
    ...
    if key_code not in keys:
        return jsonify({"valid": False, "error": "Clé invalide"}), 200
    ...
    if key_data.get("expires_at") != "Infinity":
        ...
        if expires_at < datetime.now(timezone.utc):
            return jsonify({"valid": False, "error": "Clé expirée"}), 200
...
def get_databases():
    ...
    user_databases = [
        db for db in databases if db.get("user_id") == request.current_user_id
    ]

    return jsonify({"success": True, "databases": user_databases}), 200
...
def upload_database():
    ...
    return jsonify(
        {"success": True, "message": "Base de données uploadée avec succès"}
    ), 201
...
def delete_database(db_id):
    ...
    for i, db in enumerate(databases):
        if db.get("id") == db_id and db.get("user_id") == request.current_user_id:
            ...
            save_json(DATABASES_FILE, databases)
            return jsonify(
                {"success": True, "message": "Base de données supprimée avec succès"}
            ), 200
...
def proxy_breachhub(endpoint):
    ...
    try:
        ...
        # Utiliser curl_cffi pour contourner Cloudflare (simulation TLS Chrome)
        response = cffi_requests.get(
            url, params=cleaned_params, timeout=20, impersonate="chrome110"
        )
                ...
                return jsonify(
                    {
                        ...
                        if response.text
                        else f"Erreur API: {response.status_code}",
                        "status_code": response.status_code,
                    }
                ), response.status_code
        ...
        print(traceback.format_exc())
        return jsonify(
            {"error": "Internal error", "message": f"Erreur interne: {str(e)}"}
        ), 500
...
def get_payment_plans():
    ...
    return jsonify({"success": True, "plans": PAYMENT_PLANS}), 200
...
def get_user_subscription():
    ...
    if user_subscription:
        ...
        if expires_at > datetime.now(timezone.utc):
            return jsonify({"success": True, "subscription": user_subscription}), 200
        else:
            # Abonnement expiré
            return jsonify(
                {"success": True, "subscription": None, "message": "Abonnement expiré"}
            ), 200
...
def process_payment():
    ...
    if payment_method not in ["paypal", "ltc"]:
        return jsonify(
            {"error": 'Méthode de paiement invalide. Utilisez "paypal" ou "ltc"'}
        ), 400
...
def get_payment_history():
    ...
    return jsonify({"success": True, "payments": user_payments}), 200
...
def cancel_subscription():
    ...
    if request.current_user_id in subscriptions:
        ...
        subscriptions[request.current_user_id]["cancelled_at"] = datetime.now(
            timezone.utc
        ).isoformat()
        save_json(SUBSCRIPTIONS_FILE, subscriptions)

        return jsonify(
            {"success": True, "message": "Abonnement annulé avec succès"}
        ), 200
